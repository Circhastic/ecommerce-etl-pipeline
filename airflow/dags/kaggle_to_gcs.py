...
BQ_TABLE_SUFFIX = "_ext"
BQ_VIEW_NAME = "events_unified_view"
...
def kaggle_gcs_bq_monthly_tables_view_dag():
    """
    ### Ecommerce Data Pipeline: Monthly Tables and Unified View
    Downloads monthly data from Kaggle, uploads to GCS, loads each month
    into a separate BigQuery table (YYYY_MM_ext), and creates/updates
    a unified view across all monthly tables.
    """
    ...
    def rename_upload_check_gcs(download_dir: str, data_interval_start=None) -> str | None:
        ...
        try:
            ...
            print(f"Checking if gs://{GCS_BUCKET}/{blob_path_relative} already exists...")
            if blob.exists():
                print(f"File already exists in GCS. Skipping upload.")
                return blob_path_relative
            else:
                print(f"File does not exist in GCS. Uploading {original_filepath} to gs://{GCS_BUCKET}/{blob_path_relative}")
    ...
    def check_if_gcs_path_exists(gcs_path: str | None) -> str:
        ...
        if gcs_path:
            ...
            return "load_gcs_to_bq_monthly"
        else:
            print("No GCS path returned. Skipping BigQuery processing.")
            return "skip_bq_processing"

    download_task_instance = download_and_extract() # returns XComArg
    ...
    load_gcs_to_bq_monthly = GCSToBigQueryOperator(
        task_id="load_gcs_to_bq_monthly",
        bucket=GCS_BUCKET,

        source_objects=[ "{{ ti.xcom_pull(task_ids='rename_upload_check_gcs_task', key='return_value') }}" ],
        destination_project_dataset_table=(
            f"{BQ_PROJECT_ID}.{BQ_DATASET}."
            "{{ data_interval_start.format('YYYY_MM') }}"
            f"{BQ_TABLE_SUFFIX}"
        ),
        schema_fields=[
    ...
    check_branch >> [load_gcs_to_bq_monthly, skip_bq_processing]

kaggle_gcs_bq_monthly_tables_view_dag()